...
namespace TestFakeItEasy
{
    ...
    [TestClass]
    public class TestRulesEngineFakeItEasy
    {
        ...
        public void TestPhysicalProductRules()
        {
            ...
            var owner = A.Fake<IOwner>();
            A.CallTo(() => owner.Name ).Returns("John Doe");
            A.CallTo(() => owner.Email ).Returns("jd@mail.com");

            // var agent = Substitute.For<IAgent>();
            var agent = A.Fake<IAgent>();
            A.CallTo(() => agent.Name ).Returns("James Bond");
            A.CallTo(() => agent.Commission ).Returns(0.1);

            // var physicalProduct = Substitute.For<IPhysicalProduct>();
            var physicalProduct = A.Fake<IPhysicalProduct>();
            A.CallTo(() => physicalProduct.Owner ).Returns(owner);
            A.CallTo(() => physicalProduct.Agent ).Returns(agent);
            A.CallTo(() => physicalProduct.Name ).Returns("Screw driver");

            // var paymentItem = Substitute.For<IPaymentItem>();
            var paymentItem = A.Fake<IPaymentItem>();
            A.CallTo(() => paymentItem.Product ).Returns(physicalProduct);
            A.CallTo(() => paymentItem.Amount ).Returns(1.01);

            var payment = A.Fake<IPayment>();
            A.CallTo(() => payment.Date ).Returns(DateTime.Now);
            A.CallTo(() => payment.PaymentItems ).Returns(new[] { paymentItem });
        ...
        {
            // var owner = Substitute.For<IOwner>();
            var owner = A.Fake<Owner>(o => o.WithArgumentsForConstructor(() => new Owner("John Doe", "jd@mail.com")));
            owner.Name.Should().Be("John Doe");
            owner.Email.Should().Be("jd@mail.com");
